...
const Workout = ({
  ...
  useEffect(() => {
    if (workout) {
      ...
      setRemainingSets({
        mainSets: [...workout.mainSets].filter(
          (set) => set.completed === false && set.missed === false
        ),
        volumeSets: [...workout.volumeSets].filter(
          (set) => set.completed === false && set.missed === false
        ),
      });
    }
  ...
  useEffect(() => {
    if (remainingSets) {
      ...
      if (
        remainingSets.mainSets.length > 0 ||
        remainingSets.volumeSets.length > 0
      ) {
        // Determine setIndex based on setType
        let setType =
          remainingSets.mainSets.length > 0 ? "mainSets" : "volumeSets";
      ...
      {workoutLoading && cyclesLoading ? (
        ...
        <section className="summary-container container-flex container-vertical container-vertical-center px-6">
          <div className="toolbar">
            <Link
              to="/dashboard"
              className="btn btn-back btn-icon-left btn-small btn-dark toolbar-left"
            >
              <FontAwesomeIcon icon={solid("caret-left")} /> back
            </Link>
            <select
              onChange={(e) => onWorkoutSelect(e)}
              className="select toolbar-right"
              name="pets"
              id="pet-select"
            >
              {workout &&
                currentCycle &&
                currentCycle.workoutsToDo.map((workoutOption) => (
                  <option
                    key={workoutOption._id}
                    value={`${workoutOption._id}`}
                    defaultValue={workoutOption._id === workout._id}
                  >{`Week ${workoutOption.week}: ${capitalize(
                    workoutOption.exercise
                  )}`}</option>
                ))}
            </select>
          </div>
          ...
          {workout && !workout.completed ? (
            <div className="summary-cards-container ">
              <SummaryCard
                title={workout && `${workout.exercise} max`}
                value={`${
                  currentCycle &&
                  workout &&
                  currentCycle.maxes[workout.exercise]
                }`}
              />
              ...
              <SummaryCard
                light={desktop}
                title="weight"
                value={currentSet.setData.weight}
              />
              <SummaryCard
                light={!desktop}
                title="reps"
                value={currentSet.setData.reps}
              />
            </div>
          ) : (
            ...
            <div className="set-actions">
              <form>
                <input
                  ...
                  value={notes}
                  type="text"
                  placeholder="notes for current set"
                />
              </form>
              <div className="buttons container-flex container-horizontal">
                <button
                  onClick={() => updateSet("missed")}
                  className="btn btn-regular btn-dark"
                >
                  {" "}
                  missed{" "}
                </button>
                <button
                  onClick={() => updateSet("completed")}

  ...
  describe("Complete a workout", () => {
    ...
    test("complete a workout", async () => {
      ...
      expect(window.location.href).toContain(
        `http://localhost/workout/${res.getCycles.data[0]._id}`
      );
      ...
      currentCycle.workoutsToDo.forEach((workout) => {
        expect(
          getByRole("option", {
            name: new RegExp(`week ${workout.week}: ${workout.exercise}`, "i"),
          })
        ).toBeTruthy();
      });
      ...
      res.getWorkout.data.forEach(async (workout, i) => {
        fireEvent.change(getByRole("combobox"), {
          target: { value: `${workout._id}` },
        });
        await waitForElement(() =>
          getByText(new RegExp(workout.exercise, "i"))
        );
        let elements = getAllByText(
          new RegExp(`${workout.mainSets[0].weight}`),
          "i"
        );
      ...
      // Select Week 1 Bench
      fireEvent.change(getByRole("combobox"), {
        target: { value: "6263089b801f9c06810c1de0" },
      });
      ...
      currentWorkout.mainSets.forEach(async (set, i) => {
        ...
        axios.put = jest
          ...
          .mockImplementationOnce(() =>
            Promise.resolve({ data: { workout: currentWorkout } })
          );

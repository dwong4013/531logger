...
const Dashboard = ({
  ...
  return (
    <Fragment>
      {authLoading && cyclesLoading && cyclesData === null && user === null ? (
        <Spinner />
      ) : (
        <Fragment>
          <section className="summary-container container-flex container-vertical container-vertical-center">
            ...
            <div className="toolbar">
              ...
              <button
                ...
                className="toolbar-right btn btn-big-action btn-primary"
                onClick={() => toggleModal()}
              >
            ...
            <div className="summary-cards-container my-2">
              <SummaryCard
                ...
                value={user && user.cyclesCompleted}
              />
              <SummaryCard
                light
                title="workouts left"
                value={cyclesData && cyclesData[0].workoutsToDo.length}
              />
              <SummaryCard
                light={desktop}
                title="current workout"
                value={cyclesData && cyclesData[0].workoutsToDo[0].exercise}
              />
              <SummaryCard
                light={!desktop}
                title="current week"
                value={cyclesData && cyclesData[0].workoutsToDo[0].week}
              />
            </div>
            <div className="section-header text-dark text-medium">Cycles</div>
            {/* Cycles Data */}
            {!cyclesLoading && cyclesData !== null ? (
              cyclesData.map((cycle, key) => (
                <CycleCard cycle={cycle} key={key} current={key === 0} />
              ))
            ) : (
              <p className="text-primary text-regular">
                No cycles available, create one to get started
              </p>
            )}
          </section>
        </Fragment>
...
export default connect(mapStateToProps, { getCycles, logout, loadUser })(
  Dashboard
);
